...
interface VariantsDimensionControlsProps {
  variantRows: VariantRow[]
  onUpdate?: (updatedRows: VariantRow[]) => void
}

export function VariantsDimensionControls({ variantRows, onUpdate }: VariantsDimensionControlsProps) {
  const { toast } = useToast()
  // Use first row's dimensions as source of truth, or default to 4096x4096
  const defaultWidth = variantRows.length > 0 && variantRows[0].output_width 
    ? variantRows[0].output_width 
    : 4096
  const defaultHeight = variantRows.length > 0 && variantRows[0].output_height 
    ? variantRows[0].output_height 
    : 4096
  ...
  // Update local state when variant rows change
  useEffect(() => {
    const newWidth = variantRows.length > 0 && variantRows[0].output_width 
      ? variantRows[0].output_width 
      : 4096
    const newHeight = variantRows.length > 0 && variantRows[0].output_height 
      ? variantRows[0].output_height 
      : 4096
    setWidth(newWidth)
    setHeight(newHeight)
  ...
  // Debounced save function - updates all variant rows with the same dimensions (global setting)
  const debouncedSave = useCallback(async (newWidth: number, newHeight: number) => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      ...
      setIsSaving(true)
      try {
        // Update all variant rows with the same dimensions (global setting)
        const updatePromises = variantRows.map(row => 
        ...
        if (failed.length > 0) {
          throw new Error(`Failed to update ${failed.length} variant row(s)`)
        }

        // Update local state with new dimensions
        const updatedRows = variantRows.map(row => ({
          ...row,
          output_width: newWidth,
          output_height: newHeight
        }))
        ...
        toast({
          ...
          description: `Set to ${newWidth} × ${newHeight} px for all variant rows`
        })
      } catch (error) {
        console.error('Failed to save dimensions:', error)
        toast({
          title: 'Save failed',
          description: error instanceof Error ? error.message : 'Failed to update dimensions',
          variant: 'destructive'
        })
      } finally {
        setIsSaving(false)
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <Tooltip>
            ...
            <TooltipContent>
              <p>These dimensions apply to all variant generations.</p>
